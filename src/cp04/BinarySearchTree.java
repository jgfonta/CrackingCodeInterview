...
{
  public myNode<T> root;      // reference to the root of this BST
  protected Comparator<T> comp;   // used for all comparisons
  private int myLevel,maxHeight;
  ...
  public String toString()
  {
	  ...
	  myLevel = 0;
	  maxHeight = 0;
      myString = rectoString(root);
    
      return myString;
  }
  
  private String rectoString(myNode<T> node)
  {
	String myString = "";
    if (node != null)
    {
      ...
      if (myLevel > maxHeight)
    	  maxHeight = myLevel;
      myString += myLevel + " Node " + (node.getInfo()).toString();
      if (node.getLeft() != null)
    	  myString += " Left points to " + (node.getLeft().getInfo()).toString();
      else
    	  myString += " Left points to null";
      if (node.getRight() != null)
      	myString += " Right points to " + (node.getRight().getInfo()).toString();
	  else
		  myString += " Right points to null";
	  myString += "\n";
      myString += rectoString(node.getLeft());
      myString += rectoString(node.getRight());
      myLevel--;
    }
    return myString;
  }
  
  private void recHeight(myNode<T> node)
  {
    if (node != null)
    {
      myLevel++;
      if (myLevel > maxHeight)
    	  maxHeight = myLevel;
      recHeight(node.getLeft());
      recHeight(node.getRight());
      myLevel--;
    }
  }

  //Returns the tallest height of the tree
  public int height()
  {
	  myLevel = 0;
	  maxHeight = 0;
  ...
  //Balances the tree and returns fullness ratio
  public double balance()
  {
	  Iterator iter = getIterator(BSTInterface.Traversal.Inorder);
	  int index = 0;
	  ...
	  while (iter.hasNext())
	  {
		  array[index] = (T) iter.next();
		  index++;
	  }
	  insertTree(0, index - 1, array, newTree);
	  newHeight = newTree.height();
	  fRatio = (double) newHeight / (double) origHeight;
	  
	  root = newTree.root;
	  return (fRatio);
  }
  
  private void insertTree(int low, int high, T[] array, BinarySearchTree newTree)
  {
	  int mid;
	  if (low == high)
	  {
		 this.remove(array[low]);
		 newTree.add(array[low]);
	  }
	  else if ((low + 1) == high)      
	  {
		 this.remove(array[low]);
		 newTree.add(array[low]);   
		 this.remove(array[high]);
		 newTree.add(array[high]);
	  }
	  else
	  {
			mid = (low + high) / 2;
			this.remove(array[mid]);
			newTree.add(array[mid]);          
			insertTree(low, (mid-1), array, newTree);         
			insertTree(mid + 1, high, array, newTree); 
	  }
  }
  ...
  private int recSize(myNode<T> node)
  ...
  // Returns the number of elements in this BST.
  {
    ...
    if (root != null)
    {
      LinkedList<myNode<T>> nodeStack = new LinkedList<myNode<T>>();
      myNode<T> currNode;
      nodeStack.push(root);
  ...
  private boolean recContains(T target, myNode<T> node)
  ...
  // comp.compare(target, i) == 0; otherwise, returns false.
 {
  ...
  private T recGet(T target, myNode<T> node)
  ...
  private myNode<T> recAdd(T element, myNode<T> node)
  // Adds element to tree rooted at node; tree retains its BST property.
  {
    if (node == null)
      // Addition place found
      node = new myNode<T>(element);
...
*/
  ...
  private T getPredecessor(myNode<T> subtree)
  // Returns the information held in the rightmost node of subtree
  {
    myNode<T> temp = subtree;
    while (temp.getRight() != null)
      temp = temp.getRight();
    return temp.getInfo();
  }

  private myNode<T> removeNode(myNode<T> node)
  ...
  private myNode<T> recRemove(T target, myNode<T> node)
  ...
  private void preOrder(myNode<T> node, LinkedList<T> q)
  ...
  private void inOrder(myNode<T> node, LinkedList<T> q)
  ...
  private void postOrder(myNode<T> node, LinkedList<T> q)
